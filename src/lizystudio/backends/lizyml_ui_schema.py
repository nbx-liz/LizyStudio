...
# --- Tune-tab search space catalog (P-0109 PR-3) ---
#
# Lifted to a module-level constant so both ``build_ui_schema()`` (UI
# rendering path) and ``ConfigMixin.get_tuning_defaults`` (P-0109
# intent/effective split, server-side default derivation) read the
# same shape without duplicate literals. Entries whose ``default_mode``
# is ``"range"`` or ``"choice"`` carry the canonical Tune defaults the
# backend exposes via ``TuningDefaults.space`` — see
# ``lizystudio.backends.lizyml.config_mixin._catalog_default_space``.
_SEARCH_SPACE_CATALOG: list[dict[str, Any]] = [
...
def build_ui_schema() -> dict[str, Any]:
    ...
    return {
        ...
        "search_space_catalog": _SEARCH_SPACE_CATALOG,
        "step_map": {
            "n_estimators": 100,
            "learning_rate": 0.001,
            "max_depth": 1,
